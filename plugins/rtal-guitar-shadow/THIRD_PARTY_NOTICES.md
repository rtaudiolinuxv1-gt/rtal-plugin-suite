...
`DOC-1.0` applies only to the original source files authored for this project.

This project depends on, links to, or is generated with third-party components that
remain under their own licenses. Those components are not relicensed under `DOC-1.0`.

## Current project-specific notes

- The DSP source is authored in Faust and uses functions from the Faust standard libraries.
...
- Linked runtime dependencies for the current standalone build include JACK and GTK/GLib-family libraries.
...
- Avoid introducing GPL or AGPL Faust library dependencies into the released DSP unless intentionally accepted.
